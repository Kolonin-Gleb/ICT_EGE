    ...
    for d2 in '0123456789ABCDEF':
        for d3 in '0123456789ABCDEF':
            ...
            if len(set(s)) == 3:
                ...
                if (int(d1, 16) % 2 == 0 and int(d2, 16) % 2 != 0 and int(d3, 16) % 2 == 0) or 
                (int(d1, 16) % 2 != 0 and int(d2, 16) % 2 == 0 and int(d3, 16) % 2 != 0):
                    counter += 1
                # Упрощённый вариант Искуственное назначение чётных нечётных
                # if (d1 in '13579BDF' and d2 in '02468ACE' and d3 in '13579BDF') or (d1 in '2468ACE' and d2 in '13579BDF' and d3 in '02468ACE'):
                #     counter += 1
...
# Свойство: Сумма 2х чисел разной чётности будет НЕЧЁТНОЙ

# На будущее:
# Когда требуется выполнить сложную проверку (вроде чередования чётности)
# Её лучше вынести в отдельную функцию!
# Или:
# Когда известно какие числа чётные и нечтные, проще указать их, чем перебирать в цикле
'''
